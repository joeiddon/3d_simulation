...
pixAngleRatio = width / 50					//the amount of pixels that one degree spreads over
step = 0.5
...
function renderMiniMap(){
	...
	ctx.moveTo(width - border - gap - mapWidth / 2, border + gap + mapHeight /2)
	ctx.strokeStyle = "red"
	ctx.lineTo(width - border - gap - mapWidth /2 +  (Math.sin(radFromDeg(cam.yaw)) * arrowLength), border + gap + mapHeight / 2 - (Math.cos(radFromDeg(cam.yaw)) * arrowLength))
	ctx.stroke()
